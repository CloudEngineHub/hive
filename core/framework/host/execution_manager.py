...
class ExecutionAlreadyRunningError(RuntimeError):
    ...
    def __init__(self, stream_id: str, active_ids: list[str]):
        ...
        super().__init__(
            f"Stream '{stream_id}' already has an active execution: {active_ids}. Concurrent executions on the same stream are not allowed."
        )
...
class ExecutionManager:
    ...
    async def execute(
        ...
        if resume_session_id:
            ...
            warnings.warn(
                "SessionStore not available, using deprecated exec_* ID format. Please ensure AgentRuntime is properly initialized.",
                DeprecationWarning,
                stacklevel=2,
            )
                ...
                while True:
                    ...
                    executor = Orchestrator(
                        ...
                        dynamic_memory_provider=(
                            self._dynamic_memory_provider_factory(execution_id) if self._dynamic_memory_provider_factory is not None else None
                        ),
                    )
