...
class TestDefaultSkillFiles:
    ...
    def test_combined_token_budget(self):
        """All default skill bodies combined should stay within budget.

        Default skills no longer go through the (retired) protocols_prompt
        path — they sit in the catalog for progressive disclosure. The
        practical injection cost today is via tool-gating
        (``tool_gating.augment_catalog_for_tools``), which pre-activates
        ``hive.browser-automation`` for any agent that has ``browser_*``
        tools. ``hive.browser-automation`` is intentionally heavy (~10k
        tokens of decision tree) — the only other remaining default
        (``hive.writing-hive-skills``) is a reference doc. The
        20000-token ceiling catches obvious
        bloat in either bucket without coupling to the current mix.
        """
        total_chars = 0
        ...
        assert approx_tokens < 20000, (
            f"Combined default skill bodies are ~{approx_tokens} tokens ({total_chars} chars), exceeding the 20000 token budget"
        )
...
class TestDefaultSkillManager:
    ...
    def test_disable_single_skill(self):
        config = SkillsConfig.from_agent_vars(default_skills={"hive.writing-hive-skills": {"enabled": False}})
        manager = DefaultSkillManager(config)
        manager.load()

        assert "hive.writing-hive-skills" not in manager.active_skill_names
...
class TestSkillsConfig:
    def test_default_is_enabled(self):
        config = SkillsConfig()
        assert config.is_default_enabled("hive.writing-hive-skills") is True

    def test_explicit_disable(self):
        config = SkillsConfig(default_skills={"hive.writing-hive-skills": DefaultSkillConfig(enabled=False)})
        assert config.is_default_enabled("hive.writing-hive-skills") is False
        assert config.is_default_enabled("hive.browser-automation") is True
    ...
    def test_from_agent_vars_basic(self):
        ...
        assert config.is_default_enabled("hive.writing-hive-skills") is True
        assert config.is_default_enabled("hive.browser-automation") is False

    def test_from_agent_vars_bool_shorthand(self):
        config = SkillsConfig.from_agent_vars(default_skills={"hive.writing-hive-skills": False})
        assert config.is_default_enabled("hive.writing-hive-skills") is False
    ...
    def test_get_default_overrides_returns_arbitrary_keys(self):
        # No remaining default skill uses {{placeholder}} overrides, but
        # the override mechanism still lets callers pass arbitrary
        # config alongside the enabled flag — round-trip it verbatim.
        config = SkillsConfig.from_agent_vars(
            default_skills={
                "hive.browser-automation": {"enabled": True, "custom_knob": 7},
            }
        )
        overrides = config.get_default_overrides("hive.browser-automation")
        assert overrides == {"custom_knob": 7}

    def test_get_default_overrides_empty(self):
        config = SkillsConfig()
        assert config.get_default_overrides("hive.writing-hive-skills") == {}

    def test_from_agent_vars_none_inputs(self):
        config = SkillsConfig.from_agent_vars(default_skills=None)
